...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->statement_execute (124)->then (sub {
    my $y = $_[0];
    test {
      ok $y;
...
} n => 3, name => 'bad statement id';

CTest {
  my ($c, $client) = @{$_[0]};
  return $client->statement_prepare ('create table foo0 (id int)')->then (sub {
...
} n => 1, name => 'execute die';

CTest {
  my ($c, $client) = @{$_[0]};
  return $client->statement_execute->catch (sub {
    my $x = $_[0];
    test {
      ok $x, $x;
    } $c;
...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->query ('create table foo1 (id int)')->then (sub {
...
for my $value (
  ...
  CTest {
    my ($c, $client) = @{$_[0]};
    my $x = $value->{i};
    return $client->query ("create table foo$x (id int)")->then (sub {
...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->query ('create table foo6 (id int)')->then (sub {
...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->query ('create table foo7 (id int)')->then (sub {
...
for my $value (
  ...
  CTest {
    my ($c, $client) = @{$_[0]};
    my $x = $value->{i};
    return $client->query ("create table bar$x (id int)")->then (sub {
...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->query ('create table foo21 (id int, name blob, data tinyint(10))')->then (sub {
...
for my $test (
  ...
  CTest {
    my ($c, $client) = @{$_[0]};
    my $x = $test->{id} + 500;
    return $client->query ("create table foo$x (id blob)")->then (sub {
      return $client->statement_prepare ("insert into foo$x (id) values (?)");
    })->then (sub {
      my $result = $_[0];
      my $statement_id = $result->packet->{statement_id};
...
CTest {
  my ($c, $client) = @{$_[0]};
  return $client->query ('create table foo25 (id int, name blob, data tinyint(10))')->then (sub {
...
for my $test (
  ...
  CTest {
    my ($c, $client) = @{$_[0]};
    my $x = $test->{id} + 300;
    return $client->query ("create table foo$x (value $test->{type} not null) charset=binary")->then (sub {
      ...
      test {
        if ($test->{optional_unsigned} and
          ...
          $test->{r}->{unsigned} = 1;
          is_deeply [map { $_->packet->{data} } @$rows], [[$test->{r}]], $result;
        } else {
          is_deeply [map { $_->packet->{data} } @$rows], [[$test->{r}]], $result;
        }
      } $c;
...
for my $test (
  ...
  CTest {
    my ($c, $client) = @{$_[0]};
    my $x = $test->{id} + 400;
    return $client->query ("create table foo$x (value $test->{type})")->then (sub {
