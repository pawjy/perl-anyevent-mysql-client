...
=head1 SYNOPSIS
  ...
  $client->connect
    ...
    return $client->send_query ('insert into hoge (id) values (12)');
  })->then (sub {
    die $_[0] if $_[0]->is_failure;
    return $client->send_query ('insert into fuga (id) values (56)');
  })->catch (sub {
    warn $_[0];
  })->then (sub {
    $client->disconnect;
...
=item $promise = $client->send_query ($query[, $row_callback])
...
array reference of packets representing the columns in the row.  The
row callback can return a promise need to be resolved before continue,
or can throw an exception.  In other words, it can be used in the same
way as the fulfill callback of a promise.  Column name/value pairs of
the rows returned by a query can be enumerated by the following code:

  $client->send_query ('SELECT * FROM table', sub {
    my $row = shift;
