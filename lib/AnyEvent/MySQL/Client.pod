...
=item $promise = $client->send_statement_prepare ($query)
...
=item $promise = $client->send_statement_execute ($statement_id, [$value1, $value2, ...][, $on_row])
...
the row callback similar to C<send_query> method's argument, except
...
=item $promise = $client->send_statement_close ($statement_id)

Send a C<COM_STMT_CLOSE> command with the specified statement ID.  It
returns a promise, which is resolved with the result object containing
whether the command has succeeded or not.

=item $promise = $client->send_statement_reset ($statement_id)
